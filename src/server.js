...
app.get('/download', (req, res) => {
    const file = __dirname + '/assets/esempi.zip';
    //console.log(file);
    res.download(file);
});
app.post('/upload', upload.single('datafile'), async (req, res) => {
    try {
        ...
        if (filePathSelect) {
            const input_from_select = __dirname + '/assets/upload/' + filePathSelect;
            ...
            const filePath = __dirname + '/assets/upload/' + fileName;
...
async function generateChart(inputData, predictedData, dates) {
    ...
    fs.writeFileSync('src/assets/plot/chart.svg', d3n.svgString());
